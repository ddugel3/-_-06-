...
*/
  ...
  analogWrite(PIN_LED, brightness(dist_raw));
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  float t=pulseIn(ECHO, HIGH, timeout) * scale;
...
float brightness(int raw){
  if(raw == 200)  return 0;
  else if(raw <200) return 255 - (raw-100) * 2.55;
  else if(raw > 200) return (raw-200) * 2.55;
}
